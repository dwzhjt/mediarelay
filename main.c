...
int main(void)
{
    ...
    while(true){
        *len = sizeof(struct sockaddr_in);
        int r = recvfrom(s, data, UINT16_MAX, MSG_WAITALL, (struct sockaddr*)peer, len);
        printf("%i\n", r);
        if(r > 0) {
            //data = realloc(data, r);
            ...
            if(data[0] == 'R') {
                ...
                //endpoints = realloc(endpoints, sizeof(struct endpoint*) * (endpoint_len + 1));
                ...
                for(size_t i = 0; i < vector_total(&v); i++) {
                    if(((struct endpoint*)vector_get(&v,i))->origin.sin_addr.s_addr == peer->sin_addr.s_addr){
                        vector_delete(&v, i);
                    }
                }
                continue;
                ...
                for(size_t i = 0; i < vector_total(&v); i++) {
                    ...
                    if(strcmp(endp->name, (char*)name) == 0) {
                        ...
                        memcpy(newdata + 2 + strlen(sname), data + 2 + str_len, (r - str_len - 2));
                        sendto(s, newdata, 2 + strlen(name) + (r - str_len - 2), 0, (struct sockaddr*)&((struct endpoint*)vector_get(&v,i))->origin, sizeof(struct sockaddr_in));
                    }
                }
